    ...
    <div className="fixed inset-0 flex bg-[#080808] text-white overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto w-full sm:ml-60 pb-20 sm:pb-0">
        <div className="px-4 sm:px-6 lg:px-10 py-6 sm:py-10 space-y-5 sm:space-y-8 max-w-7xl mx-auto">
          ...
          <div className={anim(0)}>
            <div className="flex items-start sm:items-end justify-between gap-3">
              <div className="flex items-center gap-3">
                {/* Mobile hamburger */}
                <button
                  className="sm:hidden p-2 rounded-lg border border-white/10 bg-white/5 text-white/60 hover:text-white transition"
                  onClick={() => setSidebarOpen(true)}>
                  <span className="text-base">☰</span>
                </button>
                <div>
                  <p className="text-[10px] font-mono text-indigo-400 tracking-[0.3em] uppercase mb-1">Overview</p>
                  <h1 className="text-2xl sm:text-4xl font-clash font-semibold tracking-tight">Analytics</h1>
                  <p className="text-white/30 text-xs sm:text-sm font-mono mt-1">Track file access and download patterns</p>
                </div>
              </div>
          ...
          {/* HIGHLIGHTS — stacked on mobile, side by side on md+ */}
          <div className={`grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 ${anim(200)}`}>
            {topFile && (
              <div className="bg-gradient-to-r from-green-400/10 to-transparent border border-green-400/20 rounded-2xl px-4 sm:px-6 py-4 sm:py-5 flex items-center justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  <span className="text-2xl sm:text-3xl shrink-0">🏆</span>
                  <div className="min-w-0">
                    <div className="text-[10px] font-mono text-white/30 uppercase tracking-widest">Most Accessed</div>
                    <div className="font-clash font-medium text-white mt-0.5 truncate text-sm sm:text-base">{topFile.fileName}</div>
                  </div>
                </div>
                <div className="text-right shrink-0">
                  <div className="text-2xl sm:text-3xl font-clash font-bold text-green-400"><AnimatedNumber value={topFile.downloadCount} /></div>
                  <div className="text-[10px] font-mono text-white/30">downloads</div>
                </div>
              </div>
            )}
            {peakDay && (
              <div className="bg-gradient-to-r from-purple-400/10 to-transparent border border-purple-400/20 rounded-2xl px-4 sm:px-6 py-4 sm:py-5 flex items-center justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  <span className="text-2xl sm:text-3xl shrink-0">⚡</span>
                  <div className="min-w-0">
                    <div className="text-[10px] font-mono text-white/30 uppercase tracking-widest">Peak Day</div>
                    <div className="font-clash font-medium text-white mt-0.5 text-sm sm:text-base">{peakDay.date}</div>
                  </div>
                </div>
                <div className="text-right shrink-0">
                  <div className="text-2xl sm:text-3xl font-clash font-bold text-purple-400"><AnimatedNumber value={peakDay.count} /></div>
                  <div className="text-[10px] font-mono text-white/30">downloads</div>
                </div>
              </div>
            )}
          ...
          {/* AREA CHART — Downloads Over Time */}
          <div className={`bg-[#0e0e0e] border border-white/5 rounded-2xl p-5 sm:p-8 ${anim(300)}`}>
            <div className="mb-4 sm:mb-6">
              <p className="text-[10px] font-mono text-indigo-400 tracking-widest uppercase mb-1">Timeline</p>
              <h2 className="text-base sm:text-xl font-clash font-semibold">Downloads Over Time</h2>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={accessPattern}>
                ...
                <XAxis dataKey="date" stroke="#1e1e1e" tick={{ fontSize: 9, fill: "#444" }} />
                <YAxis allowDecimals={false} stroke="#1e1e1e" tick={{ fontSize: 9, fill: "#444" }} width={28} />
          ...
          <div className={`grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 ${anim(400)}`}>

            {/* Pie Chart */}
            <div className="bg-[#0e0e0e] border border-white/5 rounded-2xl p-5 sm:p-8">
              <div className="mb-4 sm:mb-6">
                <p className="text-[10px] font-mono text-purple-400 tracking-widest uppercase mb-1">Distribution</p>
                <h2 className="text-base sm:text-xl font-clash font-semibold">Download Share by File</h2>
                <p className="text-white/20 text-xs font-mono mt-1">Which files dominate your downloads</p>
              </div>
              {/* Stack pie + legend vertically on small, side by side on sm */}
            ...
            <div className="bg-[#0e0e0e] border border-white/5 rounded-2xl p-5 sm:p-8">
              <div className="mb-4 sm:mb-6">
                ...
                <h2 className="text-base sm:text-xl font-clash font-semibold">Most Accessed Files</h2>
              ...
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={mostAccessed.slice(0, 6)} layout="vertical">
                  <XAxis type="number" stroke="#1e1e1e" tick={{ fontSize: 9, fill: "#444" }} />
                  <YAxis dataKey="fileName" type="category" width={90}
                    stroke="#1e1e1e" tick={{ fontSize: 9, fill: "#555" }} />
                  <Tooltip content={<CustomTooltip />} />
          ...
          {/* DAY OF WEEK BAR */}
          <div className={`bg-[#0e0e0e] border border-white/5 rounded-2xl p-5 sm:p-8 ${anim(500)}`}>
            <div className="mb-4 sm:mb-6">
              <p className="text-[10px] font-mono text-amber-400 tracking-widest uppercase mb-1">Patterns</p>
              <h2 className="text-base sm:text-xl font-clash font-semibold">Activity by Day of Week</h2>
              <p className="text-white/20 text-xs font-mono mt-1">Which days of the week see the most downloads</p>
            </div>
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={dowData}>
                <CartesianGrid stroke="#111" vertical={false} />
                <XAxis dataKey="day" stroke="#1e1e1e" tick={{ fontSize: 10, fill: "#555" }} />
                <YAxis allowDecimals={false} stroke="#1e1e1e" tick={{ fontSize: 9, fill: "#444" }} width={28} />
                <Tooltip content={<CustomTooltip />} />
          ...
          {/* RADAR CHART */}
          {radarData.length >= 3 && (
            <div className={`bg-[#0e0e0e] border border-white/5 rounded-2xl p-5 sm:p-8 ${anim(600)}`}>
              <div className="mb-4 sm:mb-6">
                <p className="text-[10px] font-mono text-sky-400 tracking-widest uppercase mb-1">Coverage</p>
                <h2 className="text-base sm:text-xl font-clash font-semibold">File Access Radar</h2>
                <p className="text-white/20 text-xs font-mono mt-1">Relative popularity of each file at a glance</p>
              </div>
              <ResponsiveContainer width="100%" height={260}>
                <RadarChart data={radarData}>
                  <PolarGrid stroke="#1e1e1e" />
                  <PolarAngleAxis dataKey="file" tick={{ fontSize: 10, fill: "#555" }} />
          ...
          {/* LAST 7 DAYS LINE */}
          <div className={`bg-[#0e0e0e] border border-white/5 rounded-2xl p-5 sm:p-8 ${anim(700)}`}>
            <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div>
                <p className="text-[10px] font-mono text-emerald-400 tracking-widest uppercase mb-1">Recent</p>
                <h2 className="text-base sm:text-xl font-clash font-semibold">Last 7 Days</h2>
                <p className="text-white/20 text-xs font-mono mt-1">{recentCount} downloads this week</p>
              </div>
              <span className={`self-start sm:self-auto text-xs font-mono px-4 py-1.5 rounded-full border

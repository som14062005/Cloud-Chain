...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${import.meta.env.VITE_API_URL}/files/shared`, {
    ...
    axios
      .get(`${import.meta.env.VITE_API_URL}/files/myfiles`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((res) => {
  ...
  const handleGrantAccess = () => {
    ...
    if (expiryOption === "24h")
      ...
      expiryTime = new Date(Date.now() + 604800000).toISOString();
    ...
    axios
      .post(
        `${import.meta.env.VITE_API_URL}/files/grant`,
  ...
  return (
    <div className={`fixed inset-0 flex ${bg} ${text} overflow-hidden transition-colors duration-300`}>
      ...
      {/* SIDEBAR — always fixed-positioned, slides in/out on mobile, visible on sm+ */}
      <div
        className={`
          fixed z-40
      ...
      {/* MAIN CONTENT — offset by sidebar width on sm+, full width on mobile */}
      <div className="flex-1 overflow-y-auto pb-20 sm:pb-0 w-full sm:ml-60">
        <div className="px-4 sm:px-6 lg:px-10 py-6 sm:py-10 max-w-7xl mx-auto">
          ...
          {/* Upload + Grant — stack on mobile, side by side on md+ */}

...
export default function GrantedAccess() {
  ...
  const fetchGranted = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/files/granted`, {
  ...
  const handleRevoke = async () => {
    ...
    try {
      await axios.post(
        `${import.meta.env.VITE_API_URL}/files/revoke`,
  ...
  const handleUpdateExpiry = async () => {
    ...
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_API_URL}/files/update-expiry`,
